package es.charles.nfccardreader.parser;

import es.charles.nfccardreader.enums.CommandEnum;
...
import es.charles.nfccardreader.iso7816emv.EmvTags;
import es.charles.nfccardreader.iso7816emv.EmvTerminal;
import es.charles.nfccardreader.iso7816emv.TLV;
import es.charles.nfccardreader.iso7816emv.TagAndLength;
import es.charles.nfccardreader.model.Afl;
import es.charles.nfccardreader.model.EmvCard;
import es.charles.nfccardreader.model.EmvTransactionRecord;
import es.charles.nfccardreader.model.enums.CurrencyEnum;
import es.charles.nfccardreader.utils.CommandApdu;
import es.charles.nfccardreader.utils.ResponseUtils;
import es.charles.nfccardreader.utils.TlvUtil;
import es.charles.nfccardreader.utils.TrackUtils;
...
import es.charles.nfccardreader.utils.ByteUtils;
...
public class EmvParser {
	...
	protected int getLeftPinTry() throws CommunicationException {
		...
		if (ResponseUtils.isSucceed(data)) {
			...
			if (val != null) {
				ret = ByteUtils.byteArrayToInt(val);
			}
		}
		return ret;
	}
	...
	protected byte[] parseFCIProprietaryTemplate(final byte[] pData) throws CommunicationException {
		...
		// Check SFI
		if (data != null) {
			int sfi = ByteUtils.byteArrayToInt(data);
			if (LOGGER.isDebugEnabled()) {
	...
	protected byte[] selectAID(final byte[] pAid) throws CommunicationException {
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Select AID: " + ByteUtils.bytesToString(pAid));
		}
	...
	protected boolean extractPublicData(final byte[] pAid, final String pApplicationLabel) throws CommunicationException {
		...
		if (ResponseUtils.isSucceed(data)) {
			...
			if (ret) {
				// Get AID
				String aid = ByteUtils.bytesToStringNoSpace(TlvUtil.getValue(data, EmvTags.DEDICATED_FILE_NAME));
				if (LOGGER.isDebugEnabled()) {
